...
class BuilderSingularMapsWithSetterPrefix<K, V> {
        ...
        public @java.lang.SuppressWarnings("all") BuilderSingularMapsWithSetterPrefix<K, V> build() {
            ...
            switch (((this.women$key == null) ? 0 : this.women$key.size())) {
                ...
                default:
                    ...
                    for (int $i = 0; ; ($i < this.women$key.size()) ;
                    $i++)
            ...
            if ((this.men$key != null))
                for (int $i = 0; ; ($i < ((this.men$key == null) ? 0 : this.men$key.size())) ;
            $i++)
            ...
            switch (((this.rawMap$key == null) ? 0 : this.rawMap$key.size())) {
                ...
                default:
                    ...
                    for (int $i = 0; ; ($i < this.rawMap$key.size()) ;
                    $i++)
            ...
            switch (((this.stringMap$key == null) ? 0 : this.stringMap$key.size())) {
                ...
                default:
                    ...
                    for (int $i = 0; ; ($i < this.stringMap$key.size()) ;
                    $i++)
    ...
    @Singular(value = "rawMap", ignoreNullCollections = true) Map rawMap;
    private @Singular("stringMap") Map<String, V> stringMap;

    @java.lang.SuppressWarnings("all")
    BuilderSingularMapsWithSetterPrefix(final Map<K, V> women, final SortedMap<K, ? extends Number> men, final Map rawMap, final Map<String, V> stringMap) {
        super();
        this.women = women;
        this.men = men;
        this.rawMap = rawMap;
